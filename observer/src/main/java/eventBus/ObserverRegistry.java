...
public class ObserverRegistry {
  ...
  public void register(Object observer) {
    ...
    for (Entry<Class<?>, Collection<ObserverAction>> entry : observerActions.entrySet()) {
      Class<?> eventType = entry.getKey();
      Collection<ObserverAction> eventActions = entry.getValue();
      registry
          .computeIfAbsent(eventType, e -> new CopyOnWriteArraySet<ObserverAction>())
          .addAll(eventActions);
    }
  }
  ...
  public List<ObserverAction> getMatchedObserverActions(Object event) {
    ...
    for (Entry<Class<?>, CopyOnWriteArraySet<ObserverAction>> entry : registry.entrySet()) {
      Class<?> eventType = entry.getKey();
